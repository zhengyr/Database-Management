...
public abstract class Node{
	...
	public int findKeyIndex (int val){
		int i = 1;
		if(val >= keys[lastindex]) return lastindex; //if value is bigger than last value in keys
		while(val > keys[i]) i++; //increasing i until we find the value
		return i;
	}
	...
	public int findPtrIndex (int val){
		int index = 0;
		if(val < keys[1]) return index; //check if it is the smallest
		if(val >= keys[lastindex]) return lastindex; //check if it is the largest
		while(val >= keys[index+1]) index++; //increasing index until we find the index
		return index ; 
	}
	...
	public void delete (int i){
		deleteSimple(i);//first delete the value and then see if we need to redistribute or combine the node
		if(lastindex >= minkeys() || getParent() == null) return; //if after delete we still have a legal tree, then we are done
		//same as what's provided in the handout
		if(siblings(getNext()) && combinable(getNext())){
			combine();
		}else if(siblings(getPrev()) && combinable(getPrev())){
			getPrev().combine();
		}else if(siblings(getNext())){
			redistribute();
		}else if(siblings(getPrev())){
			getPrev().redistribute();
		}
	}
